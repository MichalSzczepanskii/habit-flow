...
- **Date Handling:** The database stores `DATE` types for `completed_date`. The application logic (Client/API) is responsible for determining *which* date to send based on the user's local time and the 3:00 AM grace period rule. The DB does not perform timezone conversions.
